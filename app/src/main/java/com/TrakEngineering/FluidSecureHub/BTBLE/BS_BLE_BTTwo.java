...
public class BS_BLE_BTTwo extends Service {
    private static final String TAG = AppConstants.LOG_TXTN_BT + "- BLE_Link_2:";
        ...
        try {
            ...
            if (extras == null) {
                ...
                if (LinkCommunicationType.equalsIgnoreCase("BT")) {
                    ...
                    BT_BLE_Constants.BT_BLE_STATUS_STR_TWO = "";
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            ...
            mBluetoothLeService.connect(BTConstants.DEVICE_ADDRESS_2);
        }
    ...
    private void displayData(String data) {
        if (data != null) {
            try {
                ...
                //Set Relay status.
                if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.equalsIgnoreCase(BTConstants.RELAY_OFF_COMMAND) && Response.contains("OFF")) {
                    RelayStatus = false;
                } else if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.equalsIgnoreCase(BTConstants.RELAY_ON_COMMAND) && Response.contains("ON")) {
                    RelayStatus = true;
    ...
    private void readPulse() {
        ...
        TimerTask tt = new TimerTask() {
            ...
            public void run() {
                ...
                String checkPulses;
                if (BT_BLE_Constants.IS_NEW_VERSION_LINK_TWO) {
                    checkPulses = "pulse";
                } else {
                    checkPulses = "pulse:";
                }

                if (!BT_BLE_Constants.BT_BLE_LINK_TWO_STATUS && AppConstants.IS_RELAY_ON_FS2) { // && !BTConstants.SwitchedBTToUDP2
                    if (countBeforeReconnectRelay2 >= 1) {
                        if (BT_BLE_Constants.BT_BLE_STATUS_STR_TWO.equalsIgnoreCase("Disconnect")) {
                            saveLastQtyInSharedPref(Constants.FS_2_PULSE);
                            if (AppConstants.GENERATE_LOGS)
                ...
                if (RelayStatus) {
                    if (BT_BLE_Constants.IS_STOP_BUTTON_PRESSED_2) {
                        BT_BLE_Constants.IS_STOP_BUTTON_PRESSED_2 = false;
                        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                relayOffCommand();
    ...
    private void checkResponse() {
        try {
            if (RelayStatus && !BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.contains(BTConstants.RELAY_OFF_COMMAND)) {
                if (RespCount < 4) {
                    RespCount++;
                } else {
                    RespCount = 0;
        ...
        try {
            new CountDownTimer(10000, 2000) {
                public void onTick(long millisUntilFinished) {
                    if (BT_BLE_Constants.BT_BLE_STATUS_STR_TWO.equalsIgnoreCase("Connected") && (BT_BLE_Constants.IS_LINK_TWO_NOTIFY_ENABLED)) {
                        BT_BLE_Constants.IS_LINK_TWO_NOTIFY_ENABLED = false;
                        isConnected = true;
                        if (AppConstants.GENERATE_LOGS)
                        ...
                        if (nextAction.equalsIgnoreCase("last1")) {
                            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if (IsThisBTTrnx && BT_BLE_Constants.IS_NEW_VERSION_LINK_TWO) {
                                        last1Command();
                                    } else {
                ...
                public void onFinish() {
                    if (BT_BLE_Constants.BT_BLE_STATUS_STR_TWO.equalsIgnoreCase("Connected")) {
                        isConnected = true;
                        if (AppConstants.GENERATE_LOGS)
                            AppConstants.writeInFile(TAG + " Link is connected.");
        ...
        try {
            ...
            new CountDownTimer(5000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.equalsIgnoreCase(BTConstants.INFO_COMMAND) && !Response.equalsIgnoreCase("")) {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.contains(BTConstants.LAST1_COMMAND) && Response.contains("records")) {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    long attempt = (4 - (millisUntilFinished / 1000));
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.contains(transactionId) && Response.contains(transactionId)) {
    ...
    private void renameCommand() {
        try {
            ...
            if (IsThisBTTrnx) {
                if (AppConstants.GENERATE_LOGS)
                    AppConstants.writeInFile(TAG + " Sending rename command to Link: " + LinkName + " (New Name: " + BTConstants.BT2_REPLACEABLE_WIFI_NAME + ")");
                mBluetoothLeService.writeCustomCharacteristic(BTConstants.RENAME_COMMAND + BTConstants.BT2_REPLACEABLE_WIFI_NAME);
            }
                ...
                if (IsResetSwitchTimeBounce.trim().equalsIgnoreCase("1") && !PulserTimingAdjust.isEmpty() && Arrays.asList(BTConstants.P_TYPES).contains(PulserTimingAdjust) && !CommonUtils.CheckDataStoredInSharedPref(BS_BLE_BTTwo.this, "storeSwitchTimeBounceFlag2")) {
                    ...
                    new CountDownTimer(4000, 1000) {
                        public void onTick(long millisUntilFinished) {
                            long attempt = (4 - (millisUntilFinished / 1000));
                            if (attempt > 0) {
                                if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.contains(BTConstants.P_TYPE_COMMAND) && Response.contains("pulser_type")) {
                                    if (AppConstants.GENERATE_LOGS)
    ...
    private void getPulserTypeCommand() {
        try {
            if (GetPulserTypeFromLINK != null) {
                if (GetPulserTypeFromLINK.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BS_BLE_BTTwo.this, "UpdatePulserType2")) {
                    ...
                    new CountDownTimer(4000, 1000) {
                        public void onTick(long millisUntilFinished) {
                            long attempt = (4 - (millisUntilFinished / 1000));
                            if (attempt > 0) {
                                if (BT_BLE_Constants.CURRENT_COMMAND_LINK_TWO.contains(BTConstants.GET_P_TYPE_COMMAND) && Response.contains("pulser_type")) {
                                    parseGetPulserTypeCommandResponse(Response.trim());
                                    if (BTConstants.IS_BT_SPP_TO_BLE_2) {
                                        rebootCommand();
                                    } else {
    ...
    private void rebootCommand() {
        try {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    BTConstants.IS_BT_SPP_TO_BLE_2 = false;
    ...
    private void parseInfoCommandResponse(String response) {
        try {
            ...
            storeUpgradeFSVersion(BS_BLE_BTTwo.this, AppConstants.UP_HOSE_ID_FS2, version);
    ...
    public class UpgradeCurrentVersionWithUpgradableVersion extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... voids) {
            ...
            try {
                ...
                response = serverHandler.PostTextData(BS_BLE_BTTwo.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
