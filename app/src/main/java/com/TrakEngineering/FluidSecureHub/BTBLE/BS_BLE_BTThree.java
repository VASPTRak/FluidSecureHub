...
public class BS_BLE_BTThree extends Service {
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            ...
            mBluetoothLeService.connect(BTConstants.deviceAddress3);
        }
    ...
    private void displayData(String data) {
        if (data != null) {
            try {
                ...
                //Set Relay status.
                if (BT_BLE_Constants.CurrentCommand_LinkThree.equalsIgnoreCase(BTConstants.relay_off_cmd) && Response.contains("OFF")) {
                    RelayStatus = false;
                } else if (BT_BLE_Constants.CurrentCommand_LinkThree.equalsIgnoreCase(BTConstants.relay_on_cmd) && Response.contains("ON")) {
                    RelayStatus = true;
    ...
    private void ReadPulse() {
        ...
        TimerTask tt = new TimerTask() {
            ...
            public void run() {
                ...
                String checkPulses;
                if (BT_BLE_Constants.isNewVersionLinkThree) {
                    checkPulses = "pulse";
                } else {
                    checkPulses = "pulse:";
                }
                ...
                if (RelayStatus) {
                    if (BT_BLE_Constants.isStopButtonPressed3) {
                        BT_BLE_Constants.isStopButtonPressed3 = false;
                        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                relayOffCommand();
        ...
        try {
            new CountDownTimer(10000, 2000) {
                public void onTick(long millisUntilFinished) {
                    if (BT_BLE_Constants.BTBLEStatusStrThree.equalsIgnoreCase("Connected") && (BT_BLE_Constants.isLinkThreeNotifyEnabled)) {
                        BT_BLE_Constants.isLinkThreeNotifyEnabled = false;
                        isConnected = true;
                        if (AppConstants.GenerateLogs)
                ...
                public void onFinish() {
                    if (BT_BLE_Constants.BTBLEStatusStrThree.equalsIgnoreCase("Connected")) {
                        isConnected = true;
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " Link is connected.");
                        if (nextAction.equalsIgnoreCase("info")) {
    ...
    /*private void UDPFunctionalityAfterBTFailure() {
        try {
            if (CommonUtils.CheckAllHTTPLinksAreFree()) {
                ...
                if (CommonUtils.isHotspotEnabled(BS_BLE_BTThree.this)) {
                    // Disable Hotspot
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " <Turning OFF the Hotspot. (if enabled)>");
                    WifiApManager wifiApManager = new WifiApManager(BS_BLE_BTThree.this);
        ...
        try {
            ...
            new CountDownTimer(5000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CurrentCommand_LinkThree.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                            ...
                            if (!TransactionId.isEmpty()) {
                                ...
                                new Handler().postDelayed(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (IsThisBTTrnx && BT_BLE_Constants.isNewVersionLinkThree) { // && (versionNumberOfLinkThree >= 145)) {
                                            last1Command();
                                        } else {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CurrentCommand_LinkThree.contains(BTConstants.last1_cmd) && Response.contains("records")) {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    long attempt = (4 - (millisUntilFinished / 1000));
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CurrentCommand_LinkThree.contains(transactionId) && Response.contains(transactionId)) {
    ...
    public void ProceedToPostTransactionCommands() {
        ...
        if (versionNumberOfLinkThree >= 145) { // Set P_Type command supported from this version onwards
            P_Type_Command();
        } else {
                ...
                if (IsResetSwitchTimeBounce.trim().equalsIgnoreCase("1") && !PulserTimingAdjust.isEmpty() && Arrays.asList(BTConstants.p_types).contains(PulserTimingAdjust) && !CommonUtils.CheckDataStoredInSharedPref(BS_BLE_BTThree.this, "storeSwitchTimeBounceFlag3")) {
                    ...
                    new CountDownTimer(4000, 1000) {
                        public void onTick(long millisUntilFinished) {
                            long attempt = (4 - (millisUntilFinished / 1000));
                            if (attempt > 0) {
                                if (BT_BLE_Constants.CurrentCommand_LinkThree.contains(BTConstants.p_type_command) && Response.contains("pulser_type")) {
                                    if (AppConstants.GenerateLogs)
                ...
                if (GetPulserTypeFromLINK.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BS_BLE_BTThree.this, "UpdatePulserType3")) {
                    ...
                    new CountDownTimer(4000, 1000) {
                        public void onTick(long millisUntilFinished) {
                            long attempt = (4 - (millisUntilFinished / 1000));
                            if (attempt > 0) {
                                if (BT_BLE_Constants.CurrentCommand_LinkThree.contains(BTConstants.get_p_type_command) && Response.contains("pulser_type")) {
                                    ParseGetPulserTypeCommandResponse(Response.trim());
                                    if (BTConstants.isBTSPPTxnContinuedWithBLE3) {
                                        RebootCommand();
                                    } else {
    ...
    private void RebootCommand() {
        try {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    BTConstants.isBTSPPTxnContinuedWithBLE3 = false;
    ...
    private void parseInfoCommandResponse(String response) {
        try {
            ...
            storeUpgradeFSVersion(BS_BLE_BTThree.this, AppConstants.UP_HoseId_fs3, version);
    ...
    private void parseLast1CommandResponse(String response) {
        try {
            ...
            SharedPreferences sharedPref = BS_BLE_BTThree.this.getSharedPreferences("storeCmtxtnid_20_record", Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sharedPref.edit();
            editor.putString("BLE_LINK3", json20txn);
            editor.apply();
            ...
            if (isOnlineTxn) {
                if (!isTransactionCompleted) {
                    ...
                    if (hoseid != null && !hoseid.trim().isEmpty()) {
                        new BS_BLE_BTThree.UpgradeCurrentVersionWithUpgradableVersion(objEntityClass).execute();
    ...
    public class UpgradeCurrentVersionWithUpgradableVersion extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... voids) {
            ...
            try {
                ...
                response = serverHandler.PostTextData(BS_BLE_BTThree.this, AppConstants.webURL, jsonData, authString);
                //----------------------------------------------------------------------------------
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                ...
                public void run() {
                    if (isHotspotDisabled) {
                        //Enable Hotspot
                        WifiApManager wifiApManager = new WifiApManager(BS_BLE_BTThree.this);
                        if (!CommonUtils.isHotspotEnabled(BS_BLE_BTThree.this) && !AppConstants.isAllLinksAreBTLinks) {
                            if (AppConstants.GenerateLogs)
