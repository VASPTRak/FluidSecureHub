...
public class BS_BLE_BTThree extends Service {
    private static final String TAG = AppConstants.LOG_TXTN_BT + "- BLE_Link_3:";
    private BLEServiceCodeThree mBluetoothLeService;
    ...
    public int countBeforeReconnectRelay3 = 0;
    ...
    DBController controller = new DBController(BS_BLE_BTThree.this);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (extras == null) {
                ...
                Thread.sleep(2000);
                AppConstants.IS_RELAY_ON_FS3 = false;
                LinkName = CommonUtils.getLinkName(2);
                if (LinkCommunicationType.equalsIgnoreCase("BT")) {
                    IsThisBTTrnx = true;
                    BT_BLE_Constants.BT_BLE_LINK_THREE_STATUS = false;
                    BT_BLE_Constants.BT_BLE_STATUS_STR_THREE = "";
    ...
    public void offlineLogicBT3() {
        try {
            ...
            EnablePrinter = offlineController.getOfflineHubDetails(BS_BLE_BTThree.this).EnablePrinter;

            minFuelLimit = OfflineConstants.getFuelLimit(BS_BLE_BTThree.this);
            if (AppConstants.GENERATE_LOGS)
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            mBluetoothLeService = ((BLEServiceCodeThree.LocalBinder) service).getService();
            if (!mBluetoothLeService.initialize()) {
                Log.e(TAG, "Unable to initialize Bluetooth");

            }
            // Automatically connects to the device upon successful start-up initialization.
            mBluetoothLeService.connect(BTConstants.DEVICE_ADDRESS_3);
        }
    ...
    private void displayData(String data) {
        if (data != null) {
            try {
                ...
                //Set Relay status.
                if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.equalsIgnoreCase(BTConstants.RELAY_OFF_COMMAND) && Response.contains("OFF")) {
                    RelayStatus = false;
                } else if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.equalsIgnoreCase(BTConstants.RELAY_ON_COMMAND) && Response.contains("ON")) {
                    RelayStatus = true;
                    AppConstants.IS_RELAY_ON_FS3 = true;
                    if (!redpulseloop_on) {
                        readPulse();
                    }
                }
    ...
    private void readPulse() {
        ...
        TimerTask tt = new TimerTask() {
            ...
            public void run() {
                ...
                String checkPulses;
                if (BT_BLE_Constants.IS_NEW_VERSION_LINK_THREE) {
                    checkPulses = "pulse";
                } else {
                    checkPulses = "pulse:";
                }

                if (!BT_BLE_Constants.BT_BLE_LINK_THREE_STATUS && AppConstants.IS_RELAY_ON_FS3) { // && !BTConstants.SwitchedBTToUDP3
                    if (countBeforeReconnectRelay3 >= 1) {
                        if (BT_BLE_Constants.BT_BLE_STATUS_STR_THREE.equalsIgnoreCase("Disconnect")) {
                            saveLastQtyInSharedPref(Constants.FS_3_PULSE);
                            if (AppConstants.GENERATE_LOGS)
                ...
                if (BTConstants.IS_RECONNECT_CALLED_3 && !BTConstants.IS_RELAY_ON_AFTER_RECONNECT_3) {
                    cancelTimer();
                ...
                if (RelayStatus) {
                    if (BT_BLE_Constants.IS_STOP_BUTTON_PRESSED_3) {
                        BT_BLE_Constants.IS_STOP_BUTTON_PRESSED_3 = false;
                        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                relayOffCommand();
                ...
                if (Response.contains(checkPulses) && RelayStatus) {
                    ...
                    if (stopCount >= autoStopSeconds) {
                        if (Pulses <= 0) {
                            CommonUtils.upgradeTransactionStatusToSqlite(TransactionId, "4", BS_BLE_BTThree.this);
                        }
                        if (AppConstants.GENERATE_LOGS)
    ...
    private void pulseCount() {
        try {
            ...
            Constants.FS_3_GALLONS = (precision.format(fillqty));
            Constants.FS_3_PULSE = outputQuantity;
    ...
    private void updateTransactionToSqlite(String outputQuantity) {
        ...
        authEntityClass.AppInfo = " Version:" + CommonUtils.getVersionCode(BS_BLE_BTThree.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE + " " + "--Main Transaction--";
        ...
        String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BS_BLE_BTThree.this) + ":" + userEmail + ":" + "TransactionComplete" + AppConstants.LANG_PARAM);
    ...
    private String addStoredQtyToCurrentQty(String outputQuantity) {
        ...
        try {
            ...
            if (BTConstants.IS_RELAY_ON_AFTER_RECONNECT_3) {
    ...
    private void checkResponse() {
        try {
            if (RelayStatus && !BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.contains(BTConstants.RELAY_OFF_COMMAND)) {
                if (RespCount < 4) {
                    RespCount++;
                } else {
                    RespCount = 0;
        ...
        try {
            new CountDownTimer(10000, 2000) {
                public void onTick(long millisUntilFinished) {
                    if (BT_BLE_Constants.BT_BLE_STATUS_STR_THREE.equalsIgnoreCase("Connected") && (BT_BLE_Constants.IS_LINK_THREE_NOTIFY_ENABLED)) {
                        BT_BLE_Constants.IS_LINK_THREE_NOTIFY_ENABLED = false;
                        isConnected = true;
                        if (AppConstants.GENERATE_LOGS)
                        ...
                        if (nextAction.equalsIgnoreCase("last1")) {
                            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    AppConstants.IS_FIRST_COMMAND_SUCCESS_FS3 = false;
                                    if (IsThisBTTrnx && BT_BLE_Constants.IS_NEW_VERSION_LINK_THREE) {
                                        last1Command();
                                    } else {
                ...
                public void onFinish() {
                    if (BT_BLE_Constants.BT_BLE_STATUS_STR_THREE.equalsIgnoreCase("Connected")) {
                        isConnected = true;
                        if (AppConstants.GENERATE_LOGS)
                            AppConstants.writeInFile(TAG + " Link is connected.");
    ...
    private void terminateBTTxnAfterInterruption() {
        try {
            ...
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " Link not connected.");
            BTConstants.IS_RECONNECT_CALLED_3 = false;
            AppConstants.TRANSACTION_FAILED_COUNT_3++;
            AppConstants.IS_TRANSACTION_FAILED_3 = true;
        ...
        try {
            ...
            new CountDownTimer(5000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.equalsIgnoreCase(BTConstants.INFO_COMMAND) && !Response.equalsIgnoreCase("")) {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.contains(BTConstants.LAST1_COMMAND) && Response.contains("records")) {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    long attempt = (4 - (millisUntilFinished / 1000));
                    if (attempt > 0) {
                        if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.contains(transactionId) && Response.contains(transactionId)) {
        ...
        try {
            ...
            new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (RelayStatus) {
                            BTConstants.IS_RELAY_ON_AFTER_RECONNECT_3 = isAfterReconnect;
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (!RelayStatus) {
                            ...
                            if (!AppConstants.IS_RELAY_ON_FS3) {
                                infoCommand();
                            }
                            cancel();
                        } else {
    ...
    private void transactionCompleteFunction() {
        ...
        if (isOnlineTxn) {
            if (BTConstants.BT3_REPLACEABLE_WIFI_NAME == null) {
                BTConstants.BT3_REPLACEABLE_WIFI_NAME = "";
            }
            //BTLink Rename functionality
            if (BTConstants.BT3_NEED_RENAME && !BTConstants.BT3_REPLACEABLE_WIFI_NAME.isEmpty()) {
                new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        renameCommand();
    ...
    private void renameCommand() {
        try {
            ...
            if (IsThisBTTrnx) {
                if (AppConstants.GENERATE_LOGS)
                    AppConstants.writeInFile(TAG + " Sending rename command to Link: " + LinkName + " (New Name: " + BTConstants.BT3_REPLACEABLE_WIFI_NAME + ")");
                mBluetoothLeService.writeCustomCharacteristic(BTConstants.RENAME_COMMAND + BTConstants.BT3_REPLACEABLE_WIFI_NAME);
            }
            ...
            storeIsRenameFlag(this, BTConstants.BT3_NEED_RENAME, jsonData, authString);
    ...
    private void pTypeCommand() {
        ...
        try {
            if (IsResetSwitchTimeBounce != null) {
                if (IsResetSwitchTimeBounce.trim().equalsIgnoreCase("1") && !PulserTimingAdjust.isEmpty() && Arrays.asList(BTConstants.P_TYPES).contains(PulserTimingAdjust) && !CommonUtils.CheckDataStoredInSharedPref(BS_BLE_BTThree.this, "storeSwitchTimeBounceFlag3")) {
                    ...
                    new CountDownTimer(4000, 1000) {
                        public void onTick(long millisUntilFinished) {
                            long attempt = (4 - (millisUntilFinished / 1000));
                            if (attempt > 0) {
                                if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.contains(BTConstants.P_TYPE_COMMAND) && Response.contains("pulser_type")) {
                                    if (AppConstants.GENERATE_LOGS)
    ...
    private void getPulserTypeCommand() {
        try {
            if (GetPulserTypeFromLINK != null) {
                if (GetPulserTypeFromLINK.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BS_BLE_BTThree.this, "UpdatePulserType3")) {
                    ...
                    new CountDownTimer(4000, 1000) {
                        public void onTick(long millisUntilFinished) {
                            long attempt = (4 - (millisUntilFinished / 1000));
                            if (attempt > 0) {
                                if (BT_BLE_Constants.CURRENT_COMMAND_LINK_THREE.contains(BTConstants.GET_P_TYPE_COMMAND) && Response.contains("pulser_type")) {
                                    parseGetPulserTypeCommandResponse(Response.trim());
                                    if (BTConstants.IS_BT_SPP_TO_BLE_3) {
                                        rebootCommand();
                                    } else {
    ...
    private void rebootCommand() {
        try {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    BTConstants.IS_BT_SPP_TO_BLE_3 = false;
    ...
    private void stopTransaction(boolean startBackgroundServices, boolean isTransactionCompleted) {
        try {
            AppConstants.IS_TRANSACTION_COMPLETED_3 = false;
            BTConstants.IS_RELAY_ON_AFTER_RECONNECT_3 = false;
            AppConstants.clearSharedPrefByName(BS_BLE_BTThree.this, "LastQuantity_BT3");
    ...
    private void closeTransaction(boolean startBackgroundServices) {
        clearEditTextFields();
        AppConstants.IS_TRANSACTION_COMPLETED_3 = true;
        try {
            try {
    ...
    private void parseInfoCommandResponse(String response) {
        try {
            ...
            storeUpgradeFSVersion(BS_BLE_BTThree.this, AppConstants.UP_HOSE_ID_FS3, version);
    ...
    private void saveLastBTTransactionInLocalDB(String txnId, String counts) {
        try {
            ...
            authEntityClass.AppInfo = " Version:" + CommonUtils.getVersionCode(BS_BLE_BTThree.this) + " " + AppConstants.getDeviceName() + " Android " + android.os.Build.VERSION.RELEASE + " " + "--Last Transaction--";
            authEntityClass.TransactionFrom = "A";
        ...
        try {
            ...
            if (response.contains("pulser_type")) {
                ...
                if (!pulserType.isEmpty() && Arrays.asList(BTConstants.P_TYPES).contains(pulserType)) {
                    ...
                    String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BS_BLE_BTThree.this) + ":" + userEmail + ":" + "UpdatePulserTypeOfLINK" + AppConstants.LANG_PARAM);

                    UpdatePulserTypeOfLINK_entity updatePulserTypeOfLINK = new UpdatePulserTypeOfLINK_entity();
                    updatePulserTypeOfLINK.IMEIUDID = AppConstants.getIMEI(BS_BLE_BTThree.this);
                    ...
                    storePulserTypeDetails(BS_BLE_BTThree.this, jsonData, authString);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateSwitchTimeBounceForLink() {
        try {
            ...
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BS_BLE_BTThree.this) + ":" + userEmail + ":" + "UpdateSwitchTimeBounceForLink" + AppConstants.LANG_PARAM);
            ...
            storeSwitchTimeBounceFlag(BS_BLE_BTThree.this, jsonData, authString);
        ...
        try {
            if (isOnlineTxn) {
                if (!isTransactionCompleted) {
                    ...
                    objEntityClass.IMEIUDID = AppConstants.getIMEI(BS_BLE_BTThree.this);
                    objEntityClass.Email = CommonUtils.getCustomerDetails_backgroundServiceBT(BS_BLE_BTThree.this).PersonEmail;
            ...
            if (!isTransactionCompleted) {
                ...
                if (OfflineConstants.isOfflineAccess(BS_BLE_BTThree.this)) {
                    syncOfflineData();
                }
            }
        } catch (Exception e) {
    ...
    public class UpgradeCurrentVersionWithUpgradableVersion extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... voids) {
            ...
            try {
                ...
                response = serverHandler.PostTextData(BS_BLE_BTThree.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
        ...
        protected void onPostExecute(String aVoid) {
            try {
                ...
                if (ResponceMessage.equalsIgnoreCase("success")) {
                    ...
                    storeUpgradeFSVersion(BS_BLE_BTThree.this, "", "");
                }
            } catch (Exception e) {
                if (AppConstants.GENERATE_LOGS)
        ...
        if (Constants.FS_1_STATUS.equalsIgnoreCase("FREE") && Constants.FS_2_STATUS.equalsIgnoreCase("FREE") && Constants.FS_3_STATUS.equalsIgnoreCase("FREE") && Constants.FS_4_STATUS.equalsIgnoreCase("FREE") && Constants.FS_5_STATUS.equalsIgnoreCase("FREE") && Constants.FS_6_STATUS.equalsIgnoreCase("FREE")) {
            ...
            if (isOnlineTxn) {
                try {
                    ...
                    if (jArray.length() > 0) {
                        startService(new Intent(BS_BLE_BTThree.this, OffTranzSyncService.class));
                    }
