...
public class BackgroundService_AP extends Service {
        ...
        try {
            ...
            try {
                if (AppConstants.DETAILS_SERVER_SSID_LIST != null) {
                    if (AppConstants.DETAILS_SERVER_SSID_LIST.size() > 1) {
                        LinkName = AppConstants.DETAILS_SERVER_SSID_LIST.get(1).get("WifiSSId");
                    }
                }
            } catch (Exception e) {
            ...
            if (extras == null) {
                ...
                jsonRename = "{\"Request\":{\"Softap\":{\"Connect_Softap\":{\"authmode\":\"WPAPSK/WPA2PSK\",\"channel\":6,\"ssid\":\"" + AppConstants.REPLACEABLE_WIFI_NAME_FS2 + "\",\"password\":\"123456789\"}}}}";

                Constants.FS_2_STATUS = "BUSY";
                AppConstants.IS_HTTP_TXN_RUNNING_FS2 = true;

                Constants.BUSY_VEHICLE_NUMBER_LIST.add(Constants.VEHICLE_NUMBER_FS2);
                ...
                if (cd.isConnectingToInternet() && CurrTxnMode.equalsIgnoreCase("online")) {
                    ...
                    ////////////////////////////////////////////
                    String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(BackgroundService_AP.this).PersonEmail;
    ...
    public void finalLastStep() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (AppConstants.NEED_TO_RENAME_FS2) {
                    consoleString += "RENAME:\n" + jsonRename;
                    if (AppConstants.GENERATE_LOGS)
                        AppConstants.writeInFile(TAG + "Sending RENAME command to Link: " + LinkName + " (New Name: " + AppConstants.REPLACEABLE_WIFI_NAME_FS2 + ")");
        ...
        try {
            ...
            if (CurrTxnMode.equalsIgnoreCase("online")) {
                try {
                    ...
                    double DeptSum = Double.parseDouble(sharedPref.getString("DeptSum", "0"));
                    double VehPercentage = Double.parseDouble(sharedPref.getString("VehPercentage", "0"));
                    double DeptPercentage = Double.parseDouble(sharedPref.getString("DeptPercentage", "0"));
                    String SurchargeType = sharedPref.getString("SurchargeType", "");
                    double ProductPrice = Double.parseDouble(sharedPref.getString("ProductPrice", "0"));
                ...
                if (AppConstants.NEED_TO_RENAME_FS2) {
                    ...
                    rhose.SiteId = AppConstants.UP_SITE_ID_FS2;//AppConstants.R_SITE_ID;
                    rhose.HoseId = AppConstants.UP_HOSE_ID_FS2;//AppConstants.R_HOSE_ID;
                    rhose.IsHoseNameReplaced = "Y";

                    Gson gson = new Gson();
                    String jsonData = gson.toJson(rhose);

                    storeIsRenameFlag(this, AppConstants.NEED_TO_RENAME_FS2, jsonData, authString);
                }
            }
            ///////////////////////////////////
            AppConstants.BUSY_STATUS = true;
    ...
    public class SaveTankMonitorReading extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                response = serverHandler.PostTextData(BackgroundService_AP.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
    ...
    private void StoreLinkDisconnectInfo(SocketException se){
        try {
            ...
            SharedPreferences sharedPref = BackgroundService_AP.this.getSharedPreferences(AppConstants.PREF_LINK_CONNECTION_ISSUE, Context.MODE_PRIVATE);
    ...
    private void SaveLastTransactionInLocalDB(String txnid, String counts) {
        try {
            ...
            authEntityClass.AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_AP.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE + " " + "--Last Transaction--";
            authEntityClass.TransactionFrom = "A";
            authEntityClass.IsFuelingStop = IsFuelingStop;
            authEntityClass.IsLastTransaction = IsLastTransaction;
            ...
            String AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_AP.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE;
            if (AppConstants.GENERATE_LOGS)
    ...
    public void SET_PULSAR_Command() {
        try {
            ...
            String PulserTimingAdjust = calibrationPref.getString("PulserTimingAdjust_FS2", "");
            String IsResetSwitchTimeBounce = calibrationPref.getString("IsResetSwitchTimeBounce_FS2", "");
    ...
    private void updateSwitchTimeBounceForLink() {
        try {
            String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(BackgroundService_AP.this).PersonEmail;
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_AP.this) + ":" + userEmail + ":" + "UpdateSwitchTimeBounceForLink" + AppConstants.LANG_PARAM);
            ...
            pref = this.getSharedPreferences("storeSwitchTimeBounceFlag2", 0);
            editor = pref.edit();
