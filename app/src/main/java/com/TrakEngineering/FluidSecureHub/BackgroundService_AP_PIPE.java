...
public class BackgroundService_AP_PIPE extends Service {
    ...
    private static final String TAG = AppConstants.LOG_TXTN_HTTP + "-BS_FS1 ";
    String EMPTY_Val = "";
        ...
        try {
            ...
            try {
                if (AppConstants.DETAILS_SERVER_SSID_LIST != null) {
                    if (AppConstants.DETAILS_SERVER_SSID_LIST.size() > 0) {
                        LinkName = AppConstants.DETAILS_SERVER_SSID_LIST.get(0).get("WifiSSId");
                    }
                }
            } catch (Exception e) {
            ...
            if (extras == null) {
                ...
                jsonRename = "{\"Request\":{\"Softap\":{\"Connect_Softap\":{\"authmode\":\"WPAPSK/WPA2PSK\",\"channel\":6,\"ssid\":\"" + AppConstants.REPLACEABLE_WIFI_NAME_FS1 + "\",\"password\":\"123456789\"}}}}";

                Constants.FS_1_STATUS = "BUSY";
                AppConstants.IS_HTTP_TXN_RUNNING_FS1 = true;

                Constants.BUSY_VEHICLE_NUMBER_LIST.add(Constants.VEHICLE_NUMBER_FS1);
                ...
                if (cd.isConnectingToInternet() && CurrTxnMode.equalsIgnoreCase("online")) {
                    ...
                    ////////////////////////////////////////////
                    String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(BackgroundService_AP_PIPE.this).PersonEmail;
        ...
        protected void onPostExecute(String result) {
            try {
                if (jsonParam.equalsIgnoreCase(jsonRelayOff) && result.contains("relay_response")) {
                    ...
                    CommonUtils.sharedPrefTxtnInterrupted(BackgroundService_AP_PIPE.this, TransactionId, false);
    ...
    public void finalLastStep() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (AppConstants.NEED_TO_RENAME_FS1) {
                    consoleString += "RENAME:\n" + jsonRename;
                    if (AppConstants.GENERATE_LOGS)
                        AppConstants.writeInFile(TAG + "Sending RENAME command to Link: " + LinkName + " (New Name: " + AppConstants.REPLACEABLE_WIFI_NAME_FS1 + ")");
    ...
    public void getDetails() {
        ...
        odometerTenths = Constants.ODO_METER_FS1 + "";
        dNumber = Constants.DEPARTMENT_NUMBER_FS1;
        pNumber = Constants.PERSONNEL_PIN_FS1;
        oText = Constants.OTHER_FS1;
        hNumber = Constants.HOURS_FS1 + "";
    ...
    public void transactionCompleteFunction() {
        try {
            ...
            if (CurrTxnMode.equalsIgnoreCase("online")) {
                try {
                    double VehicleSum_FS1 = Double.parseDouble(sharedPref.getString("VehicleSum_FS1", "0"));
                    double DeptSum_FS1 = Double.parseDouble(sharedPref.getString("DeptSum_FS1", "0"));
                    double VehPercentage_FS1 = Double.parseDouble(sharedPref.getString("VehPercentage_FS1", "0"));
                    double DeptPercentage_FS1 = Double.parseDouble(sharedPref.getString("DeptPercentage_FS1", "0"));
                    String SurchargeType_FS1 = sharedPref.getString("SurchargeType_FS1", "");
                    double ProductPrice_FS1 = Double.parseDouble(sharedPref.getString("ProductPrice_FS1", "0"));
                ...
                if (AppConstants.NEED_TO_RENAME_FS1) {
                    ...
                    rhose.SiteId = AppConstants.UP_SITE_ID_FS1;//AppConstants.R_SITE_ID;
                    rhose.HoseId = AppConstants.UP_HOSE_ID_FS1;//AppConstants.R_HOSE_ID;
                    rhose.IsHoseNameReplaced = "Y";

                    Gson gson = new Gson();
                    String jsonData = gson.toJson(rhose);

                    storeIsRenameFlag(this, AppConstants.NEED_TO_RENAME_FS1, jsonData, authString);
                }
            }
            ///////////////////////////////////
            AppConstants.BUSY_STATUS = true;
    ...
    public class SaveTankMonitorReading extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                response = serverHandler.PostTextData(BackgroundService_AP_PIPE.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
    ...
    private void StoreLinkDisconnectInfo(SocketException se){
        try {
            ...
            SharedPreferences sharedPref = BackgroundService_AP_PIPE.this.getSharedPreferences(AppConstants.PREF_LINK_CONNECTION_ISSUE, Context.MODE_PRIVATE);
    ...
    private void SaveLastTransactionInLocalDB(String txnid, String counts) {
        try {
            ...
            authEntityClass.AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_AP_PIPE.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE + " " + "--Last Transaction--";
            authEntityClass.TransactionFrom = "A";
            authEntityClass.IsFuelingStop = IsFuelingStop;
            authEntityClass.IsLastTransaction = IsLastTransaction;
            ...
            String AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_AP_PIPE.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE;
            if (AppConstants.GENERATE_LOGS)
    ...
    public void SET_PULSAR_Command() {
        try {
            ...
            String PulserTimingAdjust = calibrationPref.getString("PulserTimingAdjust_FS1", "");
            String IsResetSwitchTimeBounce = calibrationPref.getString("IsResetSwitchTimeBounce_FS1", "");
    ...
    private void updateSwitchTimeBounceForLink() {
        try {
            String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(BackgroundService_AP_PIPE.this).PersonEmail;
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_AP_PIPE.this) + ":" + userEmail + ":" + "UpdateSwitchTimeBounceForLink" + AppConstants.LANG_PARAM);
            ...
            pref = this.getSharedPreferences("storeSwitchTimeBounceFlag1", 0);
            editor = pref.edit();
