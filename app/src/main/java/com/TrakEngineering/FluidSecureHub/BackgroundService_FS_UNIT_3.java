    ...
    public class CommandsGET extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... param) {
            ...
            try {
                ...
                Log.d("Ex", e.getMessage());
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + "  CommandsGET InBackground Exception " + e);
                stopSelf();
            }
...
//                          BackgroundService_AP.this.stopSelf();
    ...
    public synchronized void ListConnectedHotspotIPOS10_FS_UNIT_3AsyncCall() {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    e.printStackTrace();
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + "  ListConnectedHotspotIP_FS_UNIT_3 1 --Exception " + e);
                } finally {
                    try {
                        br.close();
                    } catch (IOException e) {
                        e.printStackTrace();
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + "  ListConnectedHotspotIP_FS_UNIT_3 2 --Exception " + e);
                    }
                }
            }
        });
                        ...
                        if (!line.isEmpty()) {
                            ...
                            if (splitted != null && splitted.length >= 4) {
                                ...
                                if (ip.contains(".") && mac.contains(":")) {
                                    ...
                                    listOfConnectedIP_UNIT_3.add("http://" + ip + ":80/");
                                } else {
                                    System.out.println("###IPAddress" + ip);
                                    System.out.println("###macAddress" + mac);
                                }
