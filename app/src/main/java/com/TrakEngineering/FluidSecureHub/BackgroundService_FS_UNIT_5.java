...
public class BackgroundService_FS_UNIT_5 extends Service {
        ...
        try {
            ...
            try {
                if (AppConstants.DETAILS_SERVER_SSID_LIST != null) {
                    if (AppConstants.DETAILS_SERVER_SSID_LIST.size() > 4) {
                        LinkName = AppConstants.DETAILS_SERVER_SSID_LIST.get(4).get("WifiSSId");
                    }
                }
            } catch (Exception e) {
            ...
            if (extras == null) {
                ...
                jsonRename = "{\"Request\":{\"Softap\":{\"Connect_Softap\":{\"authmode\":\"WPAPSK/WPA2PSK\",\"channel\":6,\"ssid\":\"" + AppConstants.REPLACEABLE_WIFI_NAME_FS5 + "\",\"password\":\"123456789\"}}}}";

                Constants.FS_5_STATUS = "BUSY";
                AppConstants.IS_HTTP_TXN_RUNNING_FS5 = true;

                Constants.BUSY_VEHICLE_NUMBER_LIST.add(Constants.VEHICLE_NUMBER_FS5);
                ...
                if (cd.isConnectingToInternet() && CurrTxnMode.equalsIgnoreCase("online")) {
                    ...
                    ////////////////////////////////////////////
                    String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(BackgroundService_FS_UNIT_5.this).PersonEmail;
                    String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_FS_UNIT_5.this) + ":" + userEmail + ":" + "TransactionComplete" + AppConstants.LANG_PARAM);
                ...
                if (GetPulsarAttemptFailCount == 3) {
                    if (Pulses > 0 || fillqty > 0) {
                        if (CurrTxnMode.equalsIgnoreCase("online")) {
                            CommonUtils.upgradeTransactionStatusToSqlite(TransactionId, "10", BackgroundService_FS_UNIT_5.this);
                        } else {
            ...
            if (result.contains("pulsar_status")) {
                ...
                if (!pulsar_secure_status.trim().isEmpty()) {
                    ...
                    if (secure_status == 0) {
                        ...
                        //AppConstants.colorToastBigFont(BackgroundService_FS_UNIT_5.this, AppConstants.FS5_CONNECTED_SSID + " Auto Stop!\n\nCount down timer completed.", Color.BLUE);
    ...
    public void finalLastStep() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (AppConstants.NEED_TO_RENAME_FS5) {
                    consoleString += "RENAME:\n" + jsonRename;
                    if (AppConstants.GENERATE_LOGS)
                        AppConstants.writeInFile(TAG + "Sending RENAME command to Link: " + LinkName + " (New Name: " + AppConstants.REPLACEABLE_WIFI_NAME_FS5 + ")");
    ...
    public void getDetails() {
        ...
        odometerTenths = Constants.ODO_METER_FS5 + "";
        dNumber = Constants.DEPARTMENT_NUMBER_FS5;
        pNumber = Constants.PERSONNEL_PIN_FS5;
        oText = Constants.OTHER_FS5;
        hNumber = Constants.HOURS_FS5 + "";
    ...
    public void transactionCompleteFunction() {
        try {
            ...
            if (CurrTxnMode.equalsIgnoreCase("online")) {
                try {
                    double VehicleSum_FS5 = Double.parseDouble(sharedPref.getString("VehicleSum_FS5", "0"));
                    double DeptSum_FS5 = Double.parseDouble(sharedPref.getString("DeptSum_FS5", "0"));
                    double VehPercentage_FS5 = Double.parseDouble(sharedPref.getString("VehPercentage_FS5", "0"));
                    double DeptPercentage_FS5 = Double.parseDouble(sharedPref.getString("DeptPercentage_FS5", "0"));
                    String SurchargeType_FS5 = sharedPref.getString("SurchargeType_FS5", "");
                    double ProductPrice_FS5 = Double.parseDouble(sharedPref.getString("ProductPrice_FS5", "0"));
                ...
                if (AppConstants.NEED_TO_RENAME_FS5) {
                    String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(this).PersonEmail;
                    ...
                    rhose.SiteId = AppConstants.UP_SITE_ID_FS5;//AppConstants.R_SITE_ID;
                    rhose.HoseId = AppConstants.UP_HOSE_ID_FS5;//AppConstants.R_HOSE_ID;
                    rhose.IsHoseNameReplaced = "Y";

                    Gson gson = new Gson();
                    String jsonData = gson.toJson(rhose);

                    storeIsRenameFlag(this, AppConstants.NEED_TO_RENAME_FS5, jsonData, authString);
                }
            }
            ///////////////////////////////////
            AppConstants.BUSY_STATUS = true;
    ...
    public class SaveTankMonitorReading extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                response = serverHandler.PostTextData(BackgroundService_FS_UNIT_5.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
    ...
    private void StoreLinkDisconnectInfo(SocketException se){
        try {
            ...
            SharedPreferences sharedPref = BackgroundService_FS_UNIT_5.this.getSharedPreferences(AppConstants.PREF_LINK_CONNECTION_ISSUE, Context.MODE_PRIVATE);
    ...
    private void SaveLastTransactionInLocalDB(String txnid, String counts) {
        try {
            ...
            authEntityClass.AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_FS_UNIT_5.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE + " " + "--Last Transaction--";
            authEntityClass.TransactionFrom = "A";
            authEntityClass.IsFuelingStop = IsFuelingStop;
            authEntityClass.IsLastTransaction = IsLastTransaction;
            ...
            String AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_FS_UNIT_5.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE;
            if (AppConstants.GENERATE_LOGS)
    ...
    public void SET_PULSAR_Command() {
        try {
            ...
            String PulserTimingAdjust = calibrationPref.getString("PulserTimingAdjust_FS5", "");
            String IsResetSwitchTimeBounce = calibrationPref.getString("IsResetSwitchTimeBounce_FS5", "");
    ...
    private void updateSwitchTimeBounceForLink() {
        try {
            String userEmail = CommonUtils.getCustomerDetails_backgroundServiceHTTP(BackgroundService_FS_UNIT_5.this).PersonEmail;
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_FS_UNIT_5.this) + ":" + userEmail + ":" + "UpdateSwitchTimeBounceForLink" + AppConstants.LANG_PARAM);
            ...
            pref = this.getSharedPreferences("storeSwitchTimeBounceFlag5", 0);
            editor = pref.edit();
