...
public class BackgroundService_FS_UNIT_6 extends Service {
    ...
    private static final String TAG = "BS_FS6 ";
    String EMPTY_Val = "";
    ...
    public static ArrayList<String> listOfConnectedIP_UNIT_6 = new ArrayList<String>();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (extras == null) {
                ...
                jsonRename = "{\"Request\":{\"Softap\":{\"Connect_Softap\":{\"authmode\":\"WPAPSK/WPA2PSK\",\"channel\":6,\"ssid\":\"" + AppConstants.REPLACEBLE_WIFI_NAME_FS6 + "\",\"password\":\"123456789\"}}}}";

                System.out.println("BackgroundService is on. AP_FS_PIPE" + HTTP_URL);
                Constants.FS_6STATUS = "BUSY";
                Constants.BusyVehicleNumberList.add(Constants.AccVehicleNumber_FS6);
                ...
                if (cd.isConnectingToInternet() && CurrTxnMode.equalsIgnoreCase("online")) {
                    ...
                    ////////////////////////////////////////////
                    String userEmail = CommonUtils.getCustomerDetails_backgroundService_FS6(BackgroundService_FS_UNIT_6.this).PersonEmail;
                    String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_FS_UNIT_6.this) + ":" + userEmail + ":" + "TransactionComplete");
    ...
    public class CommandsPOST extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + "  CommandsPOST OnPostExecution Execption " + e);
    ...
    public class CommandsGET extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + "  CommandsGET OnPostExecution Execption " + e);
                System.out.println(e);
                stopSelf();
            }
                    ...
                    if (stopTimer) {
                        ...
                        if (IsFsConnected(HTTP_URL)) {
                            ...
                            if (AttemptCount > 2) {
                                ...
                                stopTimer = false;
                                new BackgroundService_FS_UNIT_6.CommandsPOST().execute(URL_RELAY, jsonRelayOff);
                                Constants.FS_6STATUS = "FREE";
                                clearEditTextFields();
//                          BackgroundService_AP.this.stopSelf();
    ...
    public boolean IsFsConnected(String toMatchString) {
        ...
        for (String HttpAddress : listOfConnectedIP_UNIT_6) {
                    ...
                    if (stopAutoFuelSeconds > 0) {
                        ...
                        if (seconds >= stopAutoFuelSeconds) {
                            ...
                            if (qtyFrequencyCount()) {
                                ...
                                System.out.println("APFS_6 Auto Stop!Quantity is same for last");
                                if (AppConstants.GenerateLogs)
    ...
    public void convertCountToQuantity(String counts) {
        ...
        System.out.println("APFS_6 Pulse" + outputQuantity);
        System.out.println("APFS_6 Quantity" + (fillqty));
        DecimalFormat precision = new DecimalFormat("0.00");
        Constants.FS_6Gallons = (precision.format(fillqty));
        Constants.FS_6Pulse = outputQuantity;

        System.out.println("testing update ui Back : Gallons" + Constants.FS_6Gallons + "pulse" + Constants.FS_6Pulse);
    ...
    public void GetDetails() {
        ...
        odometerTenths = Constants.AccOdoMeter_FS6 + "";
        dNumber = Constants.AccDepartmentNumber_FS6;
        pNumber = Constants.AccPersonnelPIN_FS6;
        oText = Constants.AccOther_FS6;
        hNumber = Constants.AccHours_FS6 + "";
    ...
    public void TransactionCompleteFunction() {
        ...
        if (cd.isConnectingToInternet()) {
            ...
            String hoseid = myPrefUP.getString("hoseid_fs6", "");
            String fsversion = myPrefUP.getString("fsversion_fs6", "");

            UpgradeVersionEntity objEntityClass = new UpgradeVersionEntity();
            objEntityClass.IMEIUDID = AppConstants.getIMEI(BackgroundService_FS_UNIT_6.this);
            objEntityClass.Email = CommonUtils.getCustomerDetails_backgroundService_FS6(this).PersonEmail;
            objEntityClass.HoseId = hoseid;
            objEntityClass.Version = fsversion;

            if (hoseid != null && !hoseid.trim().isEmpty()) {
                BackgroundService_FS_UNIT_6.UpgradeCurrentVersionWithUgradableVersion objUP = new BackgroundService_FS_UNIT_6.UpgradeCurrentVersionWithUgradableVersion(objEntityClass);
                objUP.execute();
            ...
            try {
                ...
                double VehicleSum_FS6 = Double.parseDouble(sharedPref.getString("VehicleSum_FS6", ""));
                double DeptSum_FS6 = Double.parseDouble(sharedPref.getString("DeptSum_FS6", ""));
                double VehPercentage_FS6 = Double.parseDouble(sharedPref.getString("VehPercentage_FS6", ""));
                double DeptPercentage_FS6 = Double.parseDouble(sharedPref.getString("DeptPercentage_FS6", ""));
                String SurchargeType_FS6 = sharedPref.getString("SurchargeType_FS6", "");
                double ProductPrice_FS6 = Double.parseDouble(sharedPref.getString("ProductPrice_FS6", ""));
            ...
            try {
                ...
                authEntityClass.TransactionFrom = "A";
                authEntityClass.AppInfo = " Version:" + CommonUtils.getVersionCode(BackgroundService_FS_UNIT_6.this) + " " + AppConstants.getDeviceName() + " Android " + Build.VERSION.RELEASE + " ";
                authEntityClass.IsFuelingStop = IsFuelingStop;
                authEntityClass.IsLastTransaction = IsLastTransaction;
                ...
                System.out.println("AP_FS_6 TrazComp......" + jsonData);
                ...
                CommonUtils.LogMessage("APFS_6", "AuthTestAsyncTask ", ex);
            }
            ...
            if (AppConstants.NeedToRenameFS6) {
                ...
                rhose.HoseId = AppConstants.UP_HoseId_fs6;//AppConstants.R_HOSE_ID;
                rhose.IsHoseNameReplaced = "Y";

                Gson gson = new Gson();
                String jsonData = gson.toJson(rhose);

                storeIsRenameFlag(this, AppConstants.NeedToRenameFS6, jsonData, authString);
            ...
            try {
                ...
                //if (AppConstants.GenerateLogs)AppConstants.WriteinFile( TAG+"  InTransactionComplete jsonData " + jsonData);
                System.out.println("link6 TrazComp......" + jsonData);
            ...
            try {
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        new BackgroundService_FS_UNIT_6.CommandsPOST().execute(URL_RESET, "");
    ...
    public void storeUpgradeFSVersion(BackgroundService_FS_UNIT_6 activity, String hoseid, String fsversion) {
    ...
    public synchronized void ListConnectedHotspotIPOS10_FS_UNIT_6AsyncCall() {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (splitted != null && splitted.length >= 4) {
                    ...
                    if (ipAddress.contains(".") && macAddress.contains(":")) {
                        ...
                        listOfConnectedIP_UNIT_6.add("http://" + ipAddress + ":80/");
                        System.out.println("Details Of Connected HotspotIP" + listOfConnectedIP_UNIT_6);
                    } else {
                        System.out.println("###IPAddress" + ipAddress);
                        System.out.println("###macAddress" + macAddress);
                    }
    ...
    public synchronized void ListConnectedHotspotIP_FS_UNIT_6AsyncCall() {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    e.printStackTrace();
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + "  ListConnectedHotspotIP_FS_UNIT_6 1 --Exception " + e);
                } finally {
                    try {
                        br.close();
                    } catch (IOException e) {
                        e.printStackTrace();
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + "  ListConnectedHotspotIP_FS_UNIT_6 2 --Exception " + e);
                    }
                }
            }
        });
    ...
    private void StoreLinkDisconnectInfo(SocketException se){
        ...
        try{
            ...
            SharedPreferences sharedPref = BackgroundService_FS_UNIT_6.this.getSharedPreferences(AppConstants.LinkConnectionIssuePref, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sharedPref.edit();
