...
public class BackgroundService_FS_UNIT_4 extends Service {
        ...
        try {
            ...
            try {
                if (AppConstants.DETAILS_SERVER_SSID_LIST != null) {
                    if (AppConstants.DETAILS_SERVER_SSID_LIST.size() > 3) {
                        LinkName = AppConstants.DETAILS_SERVER_SSID_LIST.get(3).get("WifiSSId");
                    }
                }
            } catch (Exception e) {
            ...
            if (extras == null) {
                ...
                jsonRename = "{\"Request\":{\"Softap\":{\"Connect_Softap\":{\"authmode\":\"WPAPSK/WPA2PSK\",\"channel\":6,\"ssid\":\"" + AppConstants.REPLACEABLE_WIFI_NAME_FS4 + "\",\"password\":\"123456789\"}}}}";

                Constants.FS_4_STATUS = "BUSY";
                AppConstants.IS_HTTP_TXN_RUNNING_FS4 = true;

                Constants.BUSY_VEHICLE_NUMBER_LIST.add(Constants.VEHICLE_NUMBER_FS4);
    ...
    public void finalLastStep() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (AppConstants.NEED_TO_RENAME_FS4) {
                    consoleString += "RENAME:\n" + jsonRename;
                    if (AppConstants.GENERATE_LOGS)
                        AppConstants.writeInFile(TAG + "Sending RENAME command to Link: " + LinkName + " (New Name: " + AppConstants.REPLACEABLE_WIFI_NAME_FS4 + ")");
    ...
    public void getDetails() {
        ...
        odometerTenths = Constants.ODO_METER_FS4 + "";
        dNumber = Constants.DEPARTMENT_NUMBER_FS4;
        pNumber = Constants.PERSONNEL_PIN_FS4;
        oText = Constants.OTHER_FS4;
        hNumber = Constants.HOURS_FS4 + "";
        ...
        try {
            ...
            if (CurrTxnMode.equalsIgnoreCase("online")) {
                ...
                if (AppConstants.NEED_TO_RENAME_FS4) {
                    ...
                    rhose.SiteId = AppConstants.UP_SITE_ID_FS4;//AppConstants.R_SITE_ID;
                    rhose.HoseId = AppConstants.UP_HOSE_ID_FS4;//AppConstants.R_HOSE_ID;
                    rhose.IsHoseNameReplaced = "Y";

                    Gson gson = new Gson();
                    String jsonData = gson.toJson(rhose);

                    storeIsRenameFlag(this, AppConstants.NEED_TO_RENAME_FS4, jsonData, authString);
                }
            }
            ///////////////////////////////////
            AppConstants.BUSY_STATUS = true;
    ...
    public class SaveTankMonitorReading extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                response = serverHandler.PostTextData(BackgroundService_FS_UNIT_4.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
    ...
    private void StoreLinkDisconnectInfo(SocketException se){
        try {
            ...
            SharedPreferences sharedPref = BackgroundService_FS_UNIT_4.this.getSharedPreferences(AppConstants.PREF_LINK_CONNECTION_ISSUE, Context.MODE_PRIVATE);
