...
public class BackgroundService_BTFour extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (extras == null) {
                ...
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink_4: <Original Names of LINK: (" + OriginalNamesOfLinkList + ")>");
    ...
    private void BeginProcessUsingUDP(boolean isCalledAfterManualAttempt) {
        try {
            ...
            if (!isCalledAfterManualAttempt) {
                millis = 5000;
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink_4: " + getResources().getString(R.string.PleaseWaitForWifiConnect));
            ...
            new CountDownTimer(millis, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink_4: Selected Hose : " + LinkName + " & Connected Hose : " + ssid);
                ...
                public void onFinish() {
                    ...
                    ssid = ssid.replace("\"", "").trim();
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink_4: (onFinish) Selected Hose : " + LinkName + " & Connected Hose : " + ssid);
                    //if (ssid.equalsIgnoreCase(LinkName)) {
                    if (OriginalNamesOfLinkList.contains(ssid)) {
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink_4: Connected to " + ssid + " via WiFi.");
    ...
    private void BTReconnectionAttempt() {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                btReconnectIntent.putExtra("LinkPosition", 3);
    ...
    private void WaitAndProceedAfterManualWifiConnect() {
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_4: Exception in WaitAndProceedAfterManualWifiConnect: " + e.getMessage());
            TerminateBTTransaction();
            e.printStackTrace();
        }
    }
                    ...
                    if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                        ...
                        if (infoCommandAttempt > 0) {
                            ...
                            if (BTConstants.BTStatusStrFour.equalsIgnoreCase("Connected")) {
                                ...
                                BTConstants.retryConnForInfoCommand4 = true;
    ...
    private void last1Command() {
        try {
            ...
            if (IsThisBTTrnx) {
                ...
                BTSPPMain btspp = new BTSPPMain();
                btspp.send4(BTConstants.last1_cmd);
            }
    ...
    private void last20Command() {
        try {
            ...
            if (IsThisBTTrnx) {
                ...
                BTSPPMain btspp = new BTSPPMain();
                btspp.send4(BTConstants.last20_cmd);
            }
