...
public class BackgroundService_BTSix extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (extras == null) {
                ...
                stopCount = 0;
                Log.i(TAG, "-Started-");
                if (AppConstants.GenerateLogs) AppConstants.WriteinFile(TAG + " BTLink 6: -Started-");
            ...
            public void run() {
                ...
                if (Response.contains(checkPulses) && RelayStatus) {
                    ...
                    if (pulseCount > 1) { // pulseCount > 4
                        //Stop transaction
                        pulseCount();
                        /*Log.i(TAG, "BTLink 6: Transaction stopped.");
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 6: Transaction stopped.");*/
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("BlueLinkSix")) {
                    ...
                    /*if (AppConstants.isRelayON_fs6 && Response.trim().isEmpty()) {
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 6: No Response from Broadcast.");
                    }*/
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 6: Response from Link >>" + Response);

                    //Set Relay status.
                    if (Response.contains("OFF")) {
                        RelayStatus = false;
                    } else if (Response.contains("ON")) {
                        //AppConstants.WriteinFile(TAG + " BTLink 6: onReceive Response:" + Response.trim() + "; ReadPulse: " + redpulseloop_on);
                        RelayStatus = true;
    ...
    private void BTLinkUpgradeCheck() {
        try {
            ...
            if (BTConstants.CurrentTransactionIsBT) {
                if (BTConstants.CurrentSelectedLinkBT == 6) {
            ...
            if (isUpgrade) {
                ...
                if (file.exists()) { // && AppConstants.UP_Upgrade_File_name.startsWith("BT_")
                    ...
                    BTConstants.UpgradeStatusBT6 = "Started";
                    infoCommandBeforeUpgrade();
                    //upgradeCommand();

                } else {
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 6: BTLinkUpgradeCommand - File (" + AppConstants.UP_Upgrade_File_name + ") Not found.");
                    proceedToInfoCommand(false);
                }
            } else {
                proceedToInfoCommand(false);
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: BTLinkUpgradeCommand Exception:>>" + e.getMessage());
            proceedToInfoCommand(false);
        }
    }
    ...
    private void infoCommandBeforeUpgrade() {
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: Sending Info command (before upgrade) to Link: " + LinkName);
            BTSPPMain btspp = new BTSPPMain();
            btspp.send6(BTConstants.info_cmd);
            ...
            new CountDownTimer(5000, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                            ...
                            if (Response.contains("records") && Response.contains("mac_address")) {
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 6: Checking Info command response (before upgrade). Response: true");
                                BTConstants.isNewVersionLinkSix = true;
                                getVersionBeforeUpgrade(Response.trim(), true);
                                Response = "";
                            } else {
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 6: Checking Info command response (before upgrade). Response:>>" + Response.trim());
                                BTConstants.isNewVersionLinkSix = false;
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 6: Checking Info command response (before upgrade). Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                        ...
                        AppConstants.TxnFailedCount6++;
                        AppConstants.IsTransactionFailed6 = true;
                        BTConstants.UpgradeStatusBT6 = "Incomplete";
                        CloseTransaction(true);
                    }
                }
            }.start();
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: infoCommandBeforeUpgrade Exception:>>" + e.getMessage());
        }
    }
    ...
    public void getVersionBeforeUpgrade(String response, boolean isNewLink) {
        try {
            if (isNewLink) {
                ...
                String version = versionJsonArray.getString("version");
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink 6: LINK Version (Before Upgrade) >> " + version);
            } else {
                // Old Link version
                String version = "";
                if (response.contains("BTMAC")) {
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: getVersionBeforeUpgrade Exception:>>" + e.getMessage());
        }
    }
    ...
    private void upgradeCommand() {
        try {
            ...
            String LocalPath = getApplicationContext().getExternalFilesDir(AppConstants.FOLDER_BIN) + "/" + AppConstants.UP_Upgrade_File_name;
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: BTLinkUpgradeFunctionality file name: " + AppConstants.UP_Upgrade_File_name);

            File file = new File(LocalPath);
            long file_size = file.length();

            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: Sending upgrade command to Link: " + LinkName);
            BTSPPMain btspp = new BTSPPMain();
            btspp.send6(BTConstants.linkUpgrade_cmd + file_size);
            ...
            new CountDownTimer(10000, 2000) {
                ...
                public void onTick(long millisUntilFinished) {
                    if (Request.contains(BTConstants.linkUpgrade_cmd) && !upgradeResponse.isEmpty()) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 6: Checking upgrade command response. Response:>>" + upgradeResponse.trim());
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                BTConstants.UpgradeStatusBT6 = "Started";
                                BTConstants.isUpgradeInProgress_BT6 = true;
                ...
                public void onFinish() {
                    ...
                    if ((Request.contains(BTConstants.linkUpgrade_cmd) && !upgradeResponse.isEmpty()) || (BTConstants.CurrentCommand_LinkSix.contains(BTConstants.linkUpgrade_cmd) && !BTConstants.isNewVersionLinkSix)) {
                        //upgrade command success.
                        if (BTConstants.isNewVersionLinkSix) {
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 6: Checking upgrade command response. Response:>>" + upgradeResponse.trim());
                        ...
                        CommonUtils.UpgradeTransactionStatusToSqlite(TransactionId, "6", BackgroundService_BTSix.this);
                        Log.i(TAG, "BTLink 6: Failed to get upgrade command Response:>>" + upgradeResponse);
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 6: Checking upgrade command response. Response: false");
                        AppConstants.TxnFailedCount6++;
                        AppConstants.IsTransactionFailed6 = true;
                        BTConstants.UpgradeStatusBT6 = "Incomplete";
                        CloseTransaction(true);
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: upgradeCommand Exception:>>" + e.getMessage());
        }
    }
    ...
    public class UpgradeFileUploadFunctionality extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... f_url) {
            ...
            try {
                ...
                if (inputStream != null) {
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 6: Upload (" + AppConstants.UP_Upgrade_File_name + ") started...");
                    while ((amountOfBytesRead = inputStream.read(bufferBytes)) != -1) {
                        ...
                        if (BTConstants.BTStatusStrSix.equalsIgnoreCase("Connected")) {
                            ...
                            BTConstants.IsFileUploadCompleted = false;
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 6: After upgrade command (Link is not connected): Progress: " + progressValue);
                            BTConstants.UpgradeStatusBT6 = "Incomplete";
                            break;
                        }
                    }
                    inputStream.close();
                    if (BTConstants.UpgradeStatusBT6.isEmpty() || BTConstants.UpgradeStatusBT6.equalsIgnoreCase("Started")) {
                        BTConstants.UpgradeStatusBT6 = "Completed";
                    }
                }
                ...
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink 6: UpgradeFileUploadFunctionality InBackground Exception: " + e.getMessage());
            }
        ...
        protected void onPostExecute(String file_url) {
            //pd.dismiss();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: LINK Status: " + BTConstants.BTStatusStrSix);
            ...
            if (BTConstants.UpgradeStatusBT6.equalsIgnoreCase("Completed")) {
                ...
                storeUpgradeFSVersion(BackgroundService_BTSix.this, AppConstants.UP_HoseId_fs6, AppConstants.UP_FirmwareVersion);
                ...
                handler.postDelayed(new Runnable() {
                    public void run() {
                        if (BTConstants.BTStatusStrSix.equalsIgnoreCase("Connected")) {
                            ...
                            counter++;
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 6: Waiting to reconnect... (Attempt: " + counter + ")");
                            if (counter < 3) {
                                handler.postDelayed(this, delay);
                            } else {
                                CommonUtils.UpgradeTransactionStatusToSqlite(TransactionId, "6", BackgroundService_BTSix.this);
                                Log.i(TAG, "BTLink 6: Failed to connect to the link.");
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 6: Failed to connect to the link. (Status: " + BTConstants.BTStatusStrSix + ")");
                                IsThisBTTrnx = false;
                                AppConstants.TxnFailedCount6++;
                                AppConstants.IsTransactionFailed6 = true;
                                CloseTransaction(true);
