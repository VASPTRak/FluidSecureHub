...
public class BackgroundService_BTSix extends Service {
        ...
        try {
            ...
            if (extras == null) {
                ...
                if (LinkCommunicationType.equalsIgnoreCase("BT")) {
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink_6: Something went wrong in hose selection.");
                    CloseTransaction(false);
                    this.stopSelf();
                }
            }
        ...
        try {
            if (IsResetSwitchTimeBounce != null) {
                if (IsResetSwitchTimeBounce.trim().equalsIgnoreCase("1") && !PulserTimingAdjust.isEmpty() && Arrays.asList(BTConstants.p_types).contains(PulserTimingAdjust) && !CommonUtils.CheckDataStoredInSharedPref(BackgroundService_BTSix.this, "storeSwitchTimeBounceFlag6")) {
                    ...
                    if (IsThisBTTrnx) {
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink_6: Sending p_type command to Link: " + LinkName);
                        BTSPPMain btspp = new BTSPPMain();
                        ...
                        public void onTick(long millisUntilFinished) {
                            ...
                            if (attempt > 0) {
                                if (Request.contains(BTConstants.p_type_command) && Response.contains("pulser_type")) {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink_6: Checking p_type command response:>> " + Response);
                                    new Handler().postDelayed(new Runnable() {
                                        @Override
                                        public void run() {
                                    ...
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink_6: Checking p_type command response. Response: false");
                                }
                            }
                        }
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: P_Type_Command Exception:>>" + e.getMessage());
            ContinueToNextCommand();
        }
    }
    ...
    private void GetPulserTypeCommand() {
        try {
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: P_Type_Command (to get the pulser type from LINK) Exception:>>" + e.getMessage());
            ContinueToNextCommand();
        }
    }
    ...
    private void ParsePulserTypeCommandResponse(String response) {
        try {
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: Exception in ParsePulserTypeCommandResponse. response>> " + response + "; Exception>>" + e.getMessage());
        }
    }
    ...
    private void transactionIdCommand(String transactionId) {
        ...
        try {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        try {
                            ...
                            e.printStackTrace();
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink_6: transactionId command Exception. Exception: " + e.getMessage());
                        }
                    }
                }
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: transactionIdCommand Exception:>>" + e.getMessage());
        }
    }
    ...
    private void relayOnCommand(boolean isAfterReconnect) {
        try {
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: relayOnCommand Exception:>>" + e.getMessage());
        }
    }
    ...
    private void relayOffCommand() {
        ...
        try {
            ...
            e.printStackTrace();
            //if (AppConstants.GenerateLogs)AppConstants.WriteinFile(TAG + "BTLink_6: relayOffCommand Exception:>>" + e.getMessage());
        }
    }
    ...
    private void renameOnCommand() {
        try {
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: renameCommand Exception:>>" + e.getMessage());
        }
    }
    ...
    private void SaveLastBTTransactionInLocalDB(String txnId, String counts) {
        ...
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: SaveLastBTTransactionToServer Exception: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                try {
                    ...
                    Log.i(TAG, " BTLink_6: Exception while parsing date format.>> " + e.getMessage());
                }
