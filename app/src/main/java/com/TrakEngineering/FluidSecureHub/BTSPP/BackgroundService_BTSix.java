...
public class BackgroundService_BTSix extends Service {
    ...
    private void infoCommand() {
        try {
            BTConstants.IS_NEW_VERSION_LINK_SIX = false;
            ...
            if (IsThisBTTrnx) {
                ...
                btspp.send6(BTConstants.INFO_COMMAND);
            }
                            ...
                            if (!TransactionId.isEmpty()) {
                                ...
                                new Handler().postDelayed(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        //if (IsThisBTTrnx && BTConstants.IS_NEW_VERSION_LINK_SIX && CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkSix, BTConstants.SUPPORTED_LINK_VERSION_FOR_LAST1)) {
                                        //    last1Command();
                                        //} else {
                    ...
                    if (Request.equalsIgnoreCase(BTConstants.INFO_COMMAND) && !Response.equalsIgnoreCase("")) {
                        ...
                        if (infoCommandAttempt > 0) {
                            ...
                            if (BTConstants.BT_STATUS_STR_SIX.equalsIgnoreCase("Connected")) {
                                ...
                                BTConstants.RETRY_CONN_FOR_INFO_COMMAND6 = true;
    ...
    private void last1Command() {
        try {
            ...
            if (IsThisBTTrnx) {
                ...
                BTSPPMain btspp = new BTSPPMain();
                btspp.send6(BTConstants.LAST1_COMMAND);
            }
    ...
    private void relayOnCommand(boolean isAfterReconnect) {
        try {
            ...
            if (IsThisBTTrnx) {
                ...
                BTSPPMain btspp = new BTSPPMain();
                btspp.send6(BTConstants.RELAY_ON_COMMAND);
            }
    ...
    public void proceedToPostTransactionCommands() {
        ...
        if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkSix, BTConstants.SUPPORTED_LINK_VERSION_FOR_LAST20)) { // Last20 command supported from this version onwards
            last20Command();
        } else {
    ...
    private void renameCommand() {
        try {
            ...
            if (IsThisBTTrnx) {
                if (AppConstants.GENERATE_LOGS)
                    AppConstants.writeInFile(TAG + " BTLink_6: Sending rename command to Link: " + LinkName + " (New Name: " + BTConstants.BT6_REPLACEABLE_WIFI_NAME + ")");
                BTSPPMain btspp = new BTSPPMain();
                btspp.send6(BTConstants.RENAME_COMMAND + BTConstants.BT6_REPLACEABLE_WIFI_NAME);
            }
    ...
    private void last20Command() {
        try {
            ...
            if (IsThisBTTrnx) {
                ...
                BTSPPMain btspp = new BTSPPMain();
                btspp.send6(BTConstants.LAST20_COMMAND);
            }
    ...
    private void proceedToNextCommand() {
        if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkSix, BTConstants.SUPPORTED_LINK_VERSION_FOR_MO_STATUS)) { // CheckMOStatus command supported from this version onwards
            checkMOStatusCommand();
        /*} else if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkSix, BTConstants.SUPPORTED_LINK_VERSION_FOR_P_TYPE)) { // Set P_Type command supported from this version onwards
            pTypeCommand();*/
        } else {
            ...
            if (IsCheckMOStatus != null) {
                if (IsCheckMOStatus.trim().equalsIgnoreCase("True") && !MOStatusCheckFlag.isEmpty() && !CommonUtils.CheckDataStoredInSharedPref(BackgroundService_BTSix.this, "storeCheckMOStatusFlag6")) {
                    ...
                    if (IsThisBTTrnx) {
                        ...
                        BTSPPMain btspp = new BTSPPMain();
                        btspp.send6(BTConstants.CHECK_MO_STATUS_COMMAND + MOStatusCheckFlag);
                    }
            ...
            if (IsResetMOCheckFlag != null) {
                if (IsResetMOCheckFlag.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BackgroundService_BTSix.this, "storeResetMOCheckFlag6")) {
                    ...
                    if (IsThisBTTrnx) {
                        ...
                        BTSPPMain btspp = new BTSPPMain();
                        btspp.send6(BTConstants.RESET_MO_CHECK_FLAG_COMMAND);
                    }
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                ...
                public void run() {
                    if (isHotspotDisabled) {
                        ...
                        if (!CommonUtils.isHotspotEnabled(BackgroundService_BTSix.this) && !AppConstants.IS_ALL_LINKS_ARE_BT_LINKS) {
                            if (AppConstants.GENERATE_LOGS)
    ...
    private void pulseCount() {
        try {
            ...
            e.printStackTrace();
            //if (AppConstants.GENERATE_LOGS)AppConstants.writeInFile(TAG + " BTLink_6: pulse count Exception>>" + e.getMessage());
        }
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("BlueLinkSix")) {
                    ...
                    if (AppConstants.GENERATE_LOGS)
                        AppConstants.writeInFile(TAG + " BTLink_6: Response from Link >>" + Response);
    ...
    private void insertInitialTransactionToSqlite() {
        ...
        if (AppConstants.GENERATE_LOGS)
            AppConstants.writeInFile(TAG + " BTLink_6: <Transaction saved in local DB. LocalTxnId: " + sqliteID + "; LINK: " + LinkName + ">");
    ...
    private void checkResponse() {
        try {
            if (RelayStatus && !BTConstants.CURRENT_COMMAND_LINK_SIX.contains(BTConstants.RELAY_OFF_COMMAND)) {
                if (RespCount < 4) {
                    RespCount++;
                } else {
                    RespCount = 0;
    ...
    private void fdCheckCommand() {
        try {
            ...
            if (IsThisBTTrnx) {
                ...
                BTSPPMain btspp = new BTSPPMain();
                btspp.send6(BTConstants.FD_CHECK_COMMAND);
            }
        } catch (Exception ex) {
            if (AppConstants.GENERATE_LOGS)
    ...
    private void parseInfoCommandResponse(String response) {
        try {
            ...
            storeUpgradeFSVersion(BackgroundService_BTSix.this, AppConstants.UP_HOSE_ID_FS6, version);
    ...
    public class UpgradeCurrentVersionWithUpgradableVersion extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... voids) {
            ...
            try {
                ...
                response = serverHandler.PostTextData(BackgroundService_BTSix.this, AppConstants.WEB_URL, jsonData, authString);
                //----------------------------------------------------------------------------------
