...
public class BackgroundService_BTSix extends Service {
    ...
    private void BeginProcessUsingUDP() {
        try {
            ...
            loading.show();*/
            Toast.makeText(BackgroundService_BTSix.this, "Connecting to wifi please wait..", Toast.LENGTH_SHORT).show();
    ...
    private void renameOnCommand() {
        try {
            ...
            if (IsThisBTTrnx) {
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink 6: Sending rename command to Link: " + LinkName);
                BTSPPMain btspp = new BTSPPMain();
                btspp.send6(BTConstants.namecommand + BTConstants.BT6REPLACEBLE_WIFI_NAME);
            } else {
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink 6: Sending rename command (UDP) to Link: " + LinkName);
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                ...
                public void run() {
                    if (BTConstants.isHotspotDisabled) {
                        ...
                        if (!CommonUtils.isHotspotEnabled(BackgroundService_BTSix.this)) {
