...
public class BackgroundService_BTSix extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (extras == null) {
                ...
                /*String OriginalNamesOfLink = CommonUtils.getOriginalNamesOfLink(5);
                OriginalNamesOfLinkList = Arrays.asList(OriginalNamesOfLink.split(","));
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink_6: <Original Names of LINK: (" + OriginalNamesOfLinkList + ")>");*/
    ...
    /*private void BeginProcessUsingUDP(boolean isCalledAfterManualAttempt) {
        try {
            ...
            if (!isCalledAfterManualAttempt) {
                millis = 5000;
                if (AppConstants.GenerateLogs)
                    AppConstants.WriteinFile(TAG + " BTLink_6: " + getResources().getString(R.string.PleaseWaitForWifiConnect));
            ...
            new CountDownTimer(millis, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink_6: Selected Hose : " + LinkName + " & Connected Hose : " + ssid);
                ...
                public void onFinish() {
                    ...
                    ssid = ssid.replace("\"", "").trim();
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink_6: (onFinish) Selected Hose : " + LinkName + " & Connected Hose : " + ssid);
                    //if (ssid.equalsIgnoreCase(LinkName)) {
                    if (OriginalNamesOfLinkList.contains(ssid)) {
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink_6: Connected to " + ssid + " via WiFi.");
    ...
    /*private void BTReconnectionAttempt() {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                btReconnectIntent.putExtra("LinkPosition", 5);
    ...
    /*private void WaitAndProceedAfterManualWifiConnect() {
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_6: Exception in WaitAndProceedAfterManualWifiConnect: " + e.getMessage());
            TerminateBTTransaction();
            e.printStackTrace();
        }
    }*/
                            ...
                            if (!TransactionId.isEmpty()) {
                                ...
                                new Handler().postDelayed(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (IsThisBTTrnx && BTConstants.isNewVersionLinkSix && (versionNumberOfLinkSix >= 1411)) {
                                            last1Command();
                                        } else {
    ...
    public void ProceedToPostTransactionCommands() {
        ...
        if (versionNumberOfLinkSix >= 1411) { // Last20 command supported from this version onwards
            last20Command();
        } else if (versionNumberOfLinkSix >= 145) { // Set P_Type command supported from this version onwards
            P_Type_Command();
        } else {
    ...
    private void parseLast1CommandResponse(String response) {
        try {
            ...
            SharedPreferences.Editor editor = sharedPref.edit();
            editor.putString("LAST1_LINK6", json20txn);
            editor.apply();
        } catch (Exception e) {
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
