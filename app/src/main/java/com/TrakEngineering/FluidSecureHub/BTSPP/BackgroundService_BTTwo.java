...
public class BackgroundService_BTTwo extends Service {
                        ...
                        if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                            ...
                            if (!TransactionId.isEmpty()) {
                                if (Response.contains("records") && Response.contains("mac_address")) {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink 2: Info command Response. Attempt: " + (attempt) + "; Response: true");
                                    BTConstants.isNewVersionLinkTwo = true;
                                    parseInfoCommandResponseForLast20txtn(Response); // parse last 20 Txtn
                                    Response = "";
                                } else {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink 2: Info command Response. Attempt: " + (attempt) + "; Response:>>" + Response.trim());
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 2: Info command Response. Attempt: " + (attempt) + "; Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                        ...
                        if (!TransactionId.isEmpty()) {
                            if (Response.contains("records") && Response.contains("mac_address")) {
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 2: Info command Response. Attempt: 6; Response: true");
                                BTConstants.isNewVersionLinkTwo = true;
                                parseInfoCommandResponseForLast20txtn(Response); // parse last 20 Txtn
                                Response = "";
                            } else {
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 2: Info command Response. Attempt: 6; Response:>>" + Response.trim());
                                parseInfoCommandResponseForLast10txtn(Response.trim()); // parse last 10 Txtn
                            }
                            new Handler().postDelayed(new Runnable() {
                                @Override
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: Info command Response. Attempt: 6; Response: false");
                        AppConstants.IsTransactionFailed2 = true;
                        PostTransactionBackgroundTasks();
                        CloseTransaction();
                    }
            ...
            new CountDownTimer(4000, 1000) {
                    ...
                    if (attempt > 0) {
                        try {
                            ...
                            if (Request.contains(transactionId) && Response.contains(transactionId)) {
                                ...
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 2: transactionId command Response. Attempt: " + (attempt) + "; Response:>>" + Response.trim());
                                new Handler().postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                                ...
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 2: transactionId command Response. Attempt: " + (attempt) + "; Response: false");
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 2: transactionId command Exception. Attempt: " + (attempt) + "; Response: false. Exception: " + e.getMessage());
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (Request.contains(transactionId) && Response.contains(transactionId)) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: transactionId command Response. Attempt: 5; Response:>>" + Response.trim());
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                relayOnCommand(false); //RelayOn
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: transactionId command Response. Attempt: 5; Response: false");
                        PostTransactionBackgroundTasks();
                        CloseTransaction();
                    }
                }
    ...
    private void relayOnCommand(boolean isAfterReconnect) {
        try {
            if (isAfterReconnect) {
                BTConstants.isRelayOnAfterReconnect2 = true;
            }
            ...
            new CountDownTimer(4000, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (RelayStatus) {
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 2: relayOn command Response. Attempt: " + (attempt) + "; Response: ON");
                            cancel();
                        } else {
                            Log.i(TAG, "BTLink 2: Waiting for relayOn Command Response: " + millisUntilFinished / 1000 + " Response>>" + Response);
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 2: relayOn command Response. Attempt: " + (attempt) + "; Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (RelayStatus) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: relayOn command Response. Attempt: 5; Response: ON");
                    } else {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: relayOn command Response. Attempt: 5; Response: false");
            ...
            new CountDownTimer(4000, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (!RelayStatus) {
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 2: relayOff command Response. Attempt: " + (attempt) + "; Response:>>" + Response.trim());
                            cancel();
                        } else {
                            Log.i(TAG, "BTLink 2: Waiting for relayOff Command Response: " + millisUntilFinished / 1000 + " Response>>" + Response);
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 2: relayOff command Response. Attempt: " + (attempt) + "; Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (!RelayStatus) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: relayOff command Response. Attempt: 5; Response:>>" + Response.trim());
                    } else {
                        Log.i(TAG, "BTLink 2: Failed to get relayOff Command Response:>>" + Response);
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 2: relayOff command Response. Attempt: 5; Response: false");
                        PostTransactionBackgroundTasks();
                        CloseTransaction();
                    }
                }
    ...
    private void SaveLastBTTransactionToServer(String txnId, String counts) {
        ...
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 2: SaveLastBTTransactionToServer LastTXNid:" + txnId + "; LINK:" + LinkName + "; Pulses:" + Integer.parseInt(counts) + "; Qty:" + Lastqty);
        ...
        try {
            ...
            if (!Response.contains(checkPulses)) {
                ...
                if (stopCount >= stopAutoFuelSeconds) {
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 2: No response from link.");
                    stopCount = 0;
                    relayOffCommand(); //RelayOff
                    TransactionCompleteFunction();
                    CloseTransaction();
