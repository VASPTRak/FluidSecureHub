...
public class BackgroundService_BTOne extends Service {
                        ...
                        if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                            ...
                            if (!TransactionId.isEmpty()) {
                                if (Response.contains("records") && Response.contains("mac_address")) {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink 1: Info command Response. Attempt: " + (attempt) + "; Response: true");
                                    BTConstants.isNewVersionLinkOne = true;
                                    parseInfoCommandResponseForLast20txtn(Response); // parse last 20 Txtn
                                    Response = "";
                                } else {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink 1: Info command Response. Attempt: " + (attempt) + "; Response:>>" + Response.trim());
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 1: Info command Response. Attempt: " + (attempt) + "; Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (Request.equalsIgnoreCase(BTConstants.info_cmd) && !Response.equalsIgnoreCase("")) {
                        ...
                        if (!TransactionId.isEmpty()) {
                            if (Response.contains("records") && Response.contains("mac_address")) {
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 1: Info command Response. Attempt: 6; Response: true");
                                BTConstants.isNewVersionLinkOne = true;
                                parseInfoCommandResponseForLast20txtn(Response); // parse last 20 Txtn
                                Response = "";
                            } else {
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 1: Info command Response. Attempt: 6; Response:>>" + Response.trim());
                                parseInfoCommandResponseForLast10txtn(Response.trim()); // parse last 10 Txtn
                            }
                            new Handler().postDelayed(new Runnable() {
                                @Override
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: Info command Response. Attempt: 6; Response: false");
                        AppConstants.IsTransactionFailed1 = true;
                        PostTransactionBackgroundTasks();
                        CloseTransaction();
                    }
            ...
            new CountDownTimer(4000, 1000) {
                    ...
                    if (attempt > 0) {
                        try {
                            ...
                            if (Request.contains(transactionId) && Response.contains(transactionId)) {
                                ...
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 1: transactionId command Response. Attempt: " + (attempt) + "; Response:>>" + Response.trim());
                                new Handler().postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                                ...
                                if (AppConstants.GenerateLogs)
                                    AppConstants.WriteinFile(TAG + " BTLink 1: transactionId command Response. Attempt: " + (attempt) + "; Response: false");
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 1: transactionId command Exception. Attempt: " + (attempt) + "; Response: false. Exception: " + e.getMessage());
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (Request.contains(transactionId) && Response.contains(transactionId)) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: transactionId command Response. Attempt: 5; Response:>>" + Response.trim());
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                relayOnCommand(false); //RelayOn
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: transactionId command Response. Attempt: 5; Response: false");
                        PostTransactionBackgroundTasks();
                        CloseTransaction();
                    }
                }
    ...
    private void relayOnCommand(boolean isAfterReconnect) {
        try {
            if (isAfterReconnect) {
                BTConstants.isRelayOnAfterReconnect1 = true;
            }
            ...
            new CountDownTimer(4000, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (RelayStatus) {
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 1: relayOn command Response. Attempt: " + (attempt) + "; Response: ON");
                            cancel();
                        } else {
                            Log.i(TAG, "BTLink 1: Waiting for relayOn Command Response: " + millisUntilFinished / 1000 + " Response>>" + Response);
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 1: relayOn command Response. Attempt: " + (attempt) + "; Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (RelayStatus) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: relayOn command Response. Attempt: 5; Response: ON");
                    } else {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: relayOn command Response. Attempt: 5; Response: false");
            ...
            new CountDownTimer(4000, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (attempt > 0) {
                        if (!RelayStatus) {
                            ...
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 1: relayOff command Response. Attempt: " + (attempt) + "; Response:>>" + Response.trim());
                            cancel();
                        } else {
                            Log.i(TAG, "BTLink 1: Waiting for relayOff Command Response: " + millisUntilFinished / 1000 + " Response>>" + Response);
                            if (AppConstants.GenerateLogs)
                                AppConstants.WriteinFile(TAG + " BTLink 1: relayOff command Response. Attempt: " + (attempt) + "; Response: false");
                        }
                    }
                }
                ...
                public void onFinish() {
                    ...
                    if (!RelayStatus) {
                        ...
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: relayOff command Response. Attempt: 5; Response:>>" + Response.trim());
                    } else {
                        Log.i(TAG, "BTLink 1: Failed to get relayOff Command Response:>>" + Response);
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 1: relayOff command Response. Attempt: 5; Response: false");
                        PostTransactionBackgroundTasks();
                        CloseTransaction();
                    }
                }
    ...
    private void renameOnCommand() {
        try {
            ...
            Response = "";
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 1: Sending rename command to Link: " + LinkName);
            if (IsThisBTTrnx) {
                BTSPPMain btspp = new BTSPPMain();
    ...
    public void SaveLastQtyInSharedPref() {

        SharedPreferences sharedPrefLastQty = this.getSharedPreferences("LastQuantity_BT1", Context.MODE_PRIVATE);

        long current_count = Long.parseLong(String.valueOf(Pulses));

        SharedPreferences.Editor editorQty = sharedPrefLastQty.edit();
        editorQty.putLong("Last_Quantity", current_count);
        editorQty.commit();
    ...
    private void SaveLastBTTransactionToServer(String txnId, String counts) {
        ...
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 1: SaveLastBTTransactionToServer LastTXNid:" + txnId + "; LINK:" + LinkName + "; Pulses:" + Integer.parseInt(counts) + "; Qty:" + Lastqty);
        ...
        try {
            ...
            if (!Response.contains(checkPulses)) {
                ...
                if (stopCount >= stopAutoFuelSeconds) {
                    ...
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 1: No response from link.");
                    stopCount = 0;
                    relayOffCommand(); //RelayOff
                    TransactionCompleteFunction();
                    CloseTransaction();
